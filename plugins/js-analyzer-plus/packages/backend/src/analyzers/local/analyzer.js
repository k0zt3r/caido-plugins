...
export function analyze(source) {
  ...
  const add = (kind, value, offset, analyzer = 'Generic', note = '') => {
    ...
    result.items.push({kind, value, offset, analyzer, note});
  };
  ...
  const secretName = /(?:password|passwd|secret|api[_-]?key|access[_-]?token|refresh[_-]?token|private[_-]?key)$/i;
  const secret = (name, node) => {
    const value = literalValue(node);
    if (name && secretName.test(name) && value) add('secret', name + ' [redacted; ' + value.length + ' chars] @' + node.start, node.start, 'Generic', 'literal assignment; may be placeholder');
  };
  ...
  while (stack.length) {
    ...
    if (node.type === 'Property' || node.type === 'PropertyDefinition') secret(propertyName(node.key), node.value);
    if (node.type === 'VariableDeclarator') secret(propertyName(node.id), node.init);
    if (node.type === 'AssignmentExpression') secret(propertyName(node.left.property || node.left), node.right);
